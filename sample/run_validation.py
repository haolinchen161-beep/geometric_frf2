"""
DeepONet-SIREN 模态参数预测训练 — ANSYS 3D 固支板。
...
CONFIG = {
    ...
    'phase1_epochs': 2000,    # 全程阶段1, 先验证ω精度
    'frf_loss_weight': 500.0, # 阶段2 FRF重建损失权重 (物理耦合)
    'freq_min': 1.0,
    'freq_max': 8000.0,
    ...
    'optimizer': {
        'name': 'AdamW',
        'kwargs': {'lr': 0.001, 'weight_decay': 0.00005, 'betas': (0.9, 0.999)},
        'gradient_clip': 2.0,
    },
}
...
MODEL_CFG = {
    'encoder_type': 'modal',
    'encoder_kwargs': {
        'coord_dim': 3, 'point_feat_dim': 6,
        'hidden_dim': 256, 'n_modes': 2,
        'trunk_layers': 4, 'branch_layers': 2, 'siren_w0': 30.0,
        'branch_hidden_dim': 64,
        'amp_scale': 500000.0,
        'freq_min': 1.0, 'freq_max': 8000.0,
    },
    'decoder_kwargs': {},
}
...
class SimpleArgs:
    def __init__(self):
        self.batch_size = 8          # 降低批次方差, 压噪声地板
        self.seed = 42
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.fp16 = False  # SIREN+AMP→NaN
...
def main():
    print("=" * 60)
    print("DeepONet-SIREN 模态参数预测 — ANSYS 3D 固支板")
    print("=" * 60)
    args = SimpleArgs()
    ...
    with torch.no_grad():
        ...
        # 可变F时 point_frf 可能是list, 取第一个样本的MSE作为参考
        tgt = batch['point_frf']
    ...
    wandb.run.name = f"modal_plate_{time.strftime('%m%d_%H%M')}"
