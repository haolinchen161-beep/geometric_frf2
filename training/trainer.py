...
def train(args, config, model_cfg, net, dataloader, optimizer,
    ...
    # 三阶段训练: 独立回归 → 冻φ攻ω(FRF对齐) → 解冻联调
    phase1_epochs = config.get('phase1_epochs', 500)
    unfreeze_epoch = config.get('unfreeze_epoch', 1500)
    frf_weight = config.get('frf_loss_weight', 20.0)
    ...
    if not log_exists:
        log_writer.writerow(['epoch', 'train_loss', 'omega_pct', 'phi_mse', 'val_asinh_mse',
    ...
    for epoch in range(start_epoch, config.get('epochs', 500)):
        ...
        is_phase2 = epoch >= phase1_epochs
        is_phase3 = epoch >= unfreeze_epoch

        # 阶段切换: 只设requires_grad, 不碰optimizer(保留Adam动量)
        if epoch == phase1_epochs:
            _set_grad(net, 'trunk', False); _set_grad(net, 'head_phi', False)
            _log("=== 阶段2: 冻φ攻ω (FRF物理对齐) ===", logger)
            lowest = np.inf
        if epoch == unfreeze_epoch:
            _set_grad(net, 'trunk', True); _set_grad(net, 'head_phi', True)
            _log("=== 阶段3: 解冻联调 (精修峰值高度) ===", logger)
            lowest = np.inf
        ...
        for batch in dataloader:
            ...
            with torch.cuda.amp.autocast(enabled=args.fp16):
                if is_modal:
                    if is_phase2:
                        ...
                        # φ_exc符号对齐: 探测网络符号偏好, 翻转ANSYS随机符号
                        if phi_exc is not None:
                            ...
                            if b_idx is not None:
                                ...
                                dot = torch.sum(phi_scan * modal_phi, dim=1, keepdim=True)
                                phi_exc_corrected = phi_exc * torch.sign(dot + 1e-8)
                    ...
                    l_w, l_p = torch.tensor(0.0), torch.tensor(0.0)

            losses.append(loss.detach().cpu().item())
            if is_modal:
                omega_losses.append(F.mse_loss(omega_pred/40000.0, batch['modal_omega'].to(args.device)/40000.0).detach().cpu().item())
            ...
            if grad_clip is not None:
                if is_modal:
                    # Trunk/head_phi梯度天然健康, 只裁剪大权重的Branch侧
                    for m in [net.branch, net.head_modal, net.skip_omega]:
        ...
        omega_pct = np.sqrt(raw_w) * 100 if raw_w > 0 else 0  # RMSE→%
        omega_share = wgt_w / mean_loss * 100 if mean_loss > 0 else 0
        phi_share = wgt_p / mean_loss * 100 if mean_loss > 0 else 0
        _log(f"Epoch {epoch:4d} | ω_RMSE={omega_pct:.2f}% (占{omega_share:.0f}%) | φ_MSE={raw_p:.2f} (占{phi_share:.0f}%) | 总={mean_loss:.2e}", logger)
        _wandb_log({'Loss / Training': mean_loss, 'Omega_MSE': raw_w, 'Phi_MSE': raw_p,
                    'LR': optimizer.param_groups[0]['lr'], 'Epoch': epoch}, logger)
        ...
        if epoch % val_freq == 0 or epoch % int(config.get('epochs', 500) / 10) == 0:
            ...
            log_writer.writerow([epoch, f'{mean_loss:.2e}', f'{omega_pct:.3f}', f'{raw_p:.4f}', f'{val_loss:.4f}',
                                 f'{val_results.get("Amplitude MAE", 0):.4f}',
                                 f'{val_results.get("Amplitude MAPE (%)", 0):.2f}',
                                 f'{lr:.2e}'])
            log_file.flush()
            # 阶段1用train_loss(参数拟合), 阶段2用val_loss(防过拟合)
            best_metric = val_loss if is_phase2 else mean_loss
            if best_metric < lowest:
                metric_name = "val_loss" if is_phase2 else "train_loss"
                fmt = ".6f" if is_phase2 else ".2f"
                _log(f"best model ({metric_name}={best_metric:{fmt}})", logger)
                save_model(args.dir, epoch, net, optimizer, best_metric)
                lowest = best_metric
        else:
            log_writer.writerow([epoch, f'{mean_loss:.2e}', f'{omega_pct:.3f}', f'{raw_p:.4f}', '', '', '', f'{lr:.2e}'])
            ...
            if isinstance(frequencies, list):
                ...
                if phi_exc is not None:
                    ...
                    if geometry.batch is not None:
                        ...
                        dot = torch.sum(phi_scan * modal_phi, dim=1, keepdim=True)
                        phi_exc = phi_exc * torch.sign(dot + 1e-8)
