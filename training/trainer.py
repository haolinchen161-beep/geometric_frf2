...
def train(args, config, model_cfg, net, dataloader, optimizer,
    ...
    try:
      for epoch in range(start_epoch, config.get('epochs', 500)):
                    ...
                    if is_phase2:
                        ...
                        if phi_exc is not None:
                            ...
                            if b_idx is not None:
                                ...
                                dot = torch.sum(phi_scan * modal_phi, dim=1)
        ...
        _log(f"Epoch {epoch:4d} | ω_RMSE={omega_pct:.2f}% (占{omega_share:.0f}%) | φ_RMSE={phi_pct:.2f}% (占{phi_share:.0f}%) | 总={mean_loss:.2e}", logger)
        _wandb_log({'Loss / Training': mean_loss, 'Omega_RMSE%': omega_pct, 'Phi_RMSE%': phi_pct,
