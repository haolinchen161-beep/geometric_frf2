...
| 指标 | 结果 |
|------|------|
| 数据 | 3D悬臂梁, 240节点, 120频率, 200/50/50, 几何±10%变化, 有符号位移FRF, [1,600]Hz |
| 模型 | GNN GraphSAGE, 1.07M参数, LeakyReLU激活 |
| 共振频率 | 两峰随几何变化 (~49~85 Hz / ~310~530 Hz) |
| 损失函数 | Huber + 共振峰加权(α=2.0) + 频率平衡(β=0.8) |
| 频率采样 | 峰值区全取 + 背景区随机补足90点 |
| 固定端约束 | 去除固定端几何噪声 + 边界损失(权重50.0) |
