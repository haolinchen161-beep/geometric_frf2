...
class GeometricHDF5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 模态参数 (训练target)
        if 'modal_omega' in self.loaded:
            out['modal_omega'] = self.loaded['modal_omega'][idx]
            out['modal_zeta'] = self.loaded['modal_zeta'][idx]
            out['modal_phi'] = self.loaded['modal_phi'][idx]
...
def _stack_modal(batch):
    ...
    for key in ['modal_omega', 'modal_zeta', 'modal_phi']:
        if key not in batch[0]:
            return {}
    return {key: torch.stack([item[key] for item in batch]) for key in
            ['modal_omega', 'modal_zeta', 'modal_phi']}
