...
class GeometricHDF5Dataset(Dataset):
    ...
    def _normalize_frequencies(self):
        ...
        if 'frequencies' in self.loaded:
            f_min, f_max = 1.0, 600.0   # 物理频率范围 (Hz)
    ...
    def __getitem__(self, idx):
        ...
        if self.freq_sampling:
            # 峰值区域全取(间距<0.5Hz=自适应密集段), 背景区域随机补足到 freq_limit
            f_phys = (freq + 1) / 2 * (500 - 1) + 1  # 还原到 Hz
            diffs = torch.diff(f_phys)
            is_peak = torch.cat([torch.tensor([diffs[0] < 0.5]), diffs < 0.5])
            peak_idx = torch.where(is_peak)[0]
            bg_idx = torch.where(~is_peak)[0]

            n_total = min(self.freq_sampling_limit, len(freq))
            n_peak_keep = min(len(peak_idx), n_total)  # 峰值区全取(通常~70)
            n_bg_keep = n_total - n_peak_keep           # 剩余配额给背景区
            if n_bg_keep > 0 and len(bg_idx) > 0:
                n_bg = min(len(bg_idx), n_bg_keep)
                bg_sample = bg_idx[torch.randperm(len(bg_idx))[:n_bg]]
                keep_idx = torch.cat([peak_idx[:n_peak_keep], bg_sample])
            else:
                keep_idx = peak_idx[:n_peak_keep]
            freq = freq[keep_idx]
